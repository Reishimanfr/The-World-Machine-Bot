...
class Starboard {
  ...
  private async getTenorGif(link: string): Promise<string | null> {
    ...
    const request = await axios
      ...
      .catch((error) =>
        logger.error(`Failed to fetch tenor gif in StarboardHelper.ts: ${error.stack}`),
      );

    return request.data.results[0].media[0].gif.url || null;
  }

  private async getGuildData(guildId: string): Promise<[string[], IGuildConfig, string[]]> {
    const [guildEmojis, guildConfig, channelBlacklist] = await Promise.all([
      ...
      starboardBlacklistedChannels
        .findAll({ where: { guildId: guildId } })
        .then((o) => o.map((o) => o.dataValues.channelId)),
    ]);
  ...
  public async main() {
    ...
    const [emojis, config, blacklist] = await this.getGuildData(reaction.message.guild!.id);
