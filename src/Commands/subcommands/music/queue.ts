...
const queue: Subcommand = {
  ...
  callback: async (interaction: CommandInteraction, player: ExtPlayer) => {
    const queue: Track[] = player.queue;

    if (!queue.length) {
      return interaction.reply({
        embeds: [
          new EmbedBuilder().setDescription('[ The queue is empty. ]').setColor(util.embedColor),
        ],
        ephemeral: true,
      });
    }
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      queueEntries.push(
        `\`${i + 1}\`: **[${entry.info.title} - ${entry.info.author}](${
          entry.info.uri
        })**\nAdded by <@${entry.info.requester?.user.id}> | Duration: \`${formatSeconds(
