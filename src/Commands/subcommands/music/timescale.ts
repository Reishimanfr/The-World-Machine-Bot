...
const timescale: Subcommand = {
  ...
  callback: async (interaction: ChatInputCommandInteraction, player) => {
    ...
    util.addToAuditLog(player, interaction.user, `Set the timescale filter to ${scale}% speed ${pitch ? `And ${pitch}% pitch` : ''}`);
    
    interaction.reply({
      embeds: [
        new EmbedBuilder()
          .setDescription(`[ Timescale filter set to **${scale}% speed** ${pitch ? `And **${pitch}% pitch**` : ''} ]`)
          .setColor(util.embedColor)
          .setFooter({ text: 'Notice: You may need to way a bit before the filter gets set!' }),
      ],
      ephemeral: true,
    });
  },
